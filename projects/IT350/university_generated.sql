...
(
    ...
    quesstion_text       varchar(128) not null,
    -- audit
...
(
    ...
    date_sent             datetime    default current_date(),
    -- audit
    `created_date`        timestamp   default CURRENT_TIMESTAMP(),
    `last_modified_date`  timestamp   default CURRENT_TIMESTAMP(),
    `last_modified_by`    varchar(64) default 'system',
...
(
    ...
    `last_modified_by`   varchar(64) default 'system',
    `record_status`      int         default 1,
    constraint fk_test_question foreign key (question_fk)
        references question (question_id) on delete cascade on update cascade
);
...
(
    ...
    min_espb             int         not null,
    study_title          varchar(32) not null,
    -- audit
    `created_date`       timestamp   default CURRENT_TIMESTAMP(),
    `last_modified_date` timestamp   default CURRENT_TIMESTAMP(),
    `last_modified_by`   varchar(64) default 'system',
