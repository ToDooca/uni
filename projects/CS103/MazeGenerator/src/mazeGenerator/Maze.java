...
public class Maze {
    ...
    public boolean solveMaze(int sx, int sy, int ex, int ey, int px, int py) {
        // set the maze start point as visited
        if (!initialize) {
        ...
        if (sx == ex && sy == ey) {
            ...
            if (sx + 1 < mazeWidth * 2) {
                if (!maze[sx + 1][sy] && !mazeSolution[sx + 1][sy] && !(sx == px && sy == py)) {
                    ...
                    System.out.println(Arrays.deepToString(mazeSolution));
