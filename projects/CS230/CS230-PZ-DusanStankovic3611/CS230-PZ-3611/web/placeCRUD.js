...
// Model for Place entity
    models.Place = Backbone.Model.extend({
        urlRoot: "http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.place/",
        idAttribute: 'placeid',
        defaults: {
            placetype: "",
            placeadress: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('placeid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.place/';
            }
    ...
    // Collection class for Place entities
    models.PlaceCollection = Backbone.Collection.extend({
        model: models.Place,
        url: "http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.place/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PlaceCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-place-list-itemis template identifier for item
                        templateName: '#tpl-place-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.place = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.place,
                    // tpl-place-details is template identifier for chosen model element
                    templateName: '#tpl-place-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Place();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-place-details is a template identifier for chosen model element
                templateName: '#tpl-place-details',
