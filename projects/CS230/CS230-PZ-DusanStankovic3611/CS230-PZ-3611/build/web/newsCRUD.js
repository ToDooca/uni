...
// Model for News entity
    models.News = Backbone.Model.extend({
        urlRoot: "http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.news/",
        idAttribute: 'newsid',
        defaults: {
            newstitle: "",
            newstext: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('newsid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.news/';
            }
    ...
    // Collection class for News entities
    models.NewsCollection = Backbone.Collection.extend({
        model: models.News,
        url: "http://localhost:29373/CS230-PZ-3611/webresources/dusanstankovic3611.cs230.entity.news/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.NewsCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-news-list-itemis template identifier for item
                        templateName: '#tpl-news-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.news = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.news,
                    // tpl-news-details is template identifier for chosen model element
                    templateName: '#tpl-news-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.News();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-news-details is a template identifier for chosen model element
                templateName: '#tpl-news-details',
